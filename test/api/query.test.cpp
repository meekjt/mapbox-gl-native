...
TEST(Query, QueryRenderedFeaturesFilterSource) {
    QueryTest test;

    auto zz = test.map.pixelForLatLng({ 0, 0 });

    auto features1 = test.map.queryRenderedFeatures(zz, {{ "layer1"}}, {{ "source1" }});
    EXPECT_EQ(features1.size(), 1u);

    auto features2 = test.map.queryRenderedFeatures(zz, {{ "layer1", "layer2" }}, {{ "source1" }});
    EXPECT_EQ(features2.size(), 1u);

    auto features3 = test.map.queryRenderedFeatures(zz, {{ "layer1", "layer2" }}, {{ "source1", "source2" }});
    EXPECT_EQ(features3.size(), 2u);

    auto features4 = test.map.queryRenderedFeatures(zz, {{ "layer1", "layer2" }}, {{ "source1", "source3" }});
    EXPECT_EQ(features4.size(), 1u);

    auto features5 = test.map.queryRenderedFeatures(zz, {{ "layer1", "layer2" }}, {{ "source1", "foobar" }});
    EXPECT_EQ(features5.size(), 1u);

    const optional<std::vector<std::string>> layer = {};

    auto features6 = test.map.queryRenderedFeatures(zz, layer, {{ "source1", "source2" }});
    EXPECT_EQ(features6.size(), 2u);

    auto features7 = test.map.queryRenderedFeatures(zz, layer, {{ "source1", "source2", "source3" }});
    EXPECT_EQ(features7.size(), 3u);
}
