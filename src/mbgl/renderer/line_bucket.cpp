...
void LineBucket::addCurrentVertex(const GeometryCoordinate& currentVertex,
    ...
    vertexes.emplace_back(currentVertex.x, currentVertex.y, extrude.x, extrude.y, tx, 0, endLeft, distance * LINE_DISTANCE_SCALE);
    ...
    vertexes.emplace_back(currentVertex.x, currentVertex.y, extrude.x, extrude.y, tx, 1, -endRight, distance * LINE_DISTANCE_SCALE);
    e3 = vertexes.size() - 1 - startVertex;
    if (e1 >= 0 && e2 >= 0) {
        triangleStore.emplace_back(e1, e2, e3);
    }
